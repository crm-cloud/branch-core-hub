...
function GstRatesEditor({ orgSettingsId, currentRates }: { orgSettingsId?: string; currentRates: number[] | null }) {
  const queryClient = useQueryClient();
  const [rates, setRates] = useState<number[]>(currentRates && currentRates.length > 0 ? currentRates : [5, 12, 18, 28]);
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <Button onClick={() => saveMutation.mutate()} disabled={saveMutation.isPending || !orgSettingsId}>
          {saveMutation.isPending ? 'Saving...' : 'Save GST Rates'}
        </Button>
      </CardContent>
    </Card>
  );
