...
export default function StaffDashboard() {
  ...
  const { data: followUpLeads = [] } = useQuery({
    ...
    enabled: !!branchId,
    queryFn: async () => {
      const { data, error } = await supabase.from('leads').select('id, name, phone, source, status, follow_up_date, notes').eq('branch_id', branchId!).in('status', ['new', 'contacted']).order('follow_up_date', { ascending: true, nullsFirst: false }).limit(5);
      if (error) throw error;
      return data || [];
    },
  });
    ...
    <AppLayout>
      <div className="space-y-6">
        ...
        <div className="grid gap-4 grid-cols-2 md:grid-cols-4">
          ...
          <StatCard title="Pending Invoices" value={stats?.pendingInvoices || 0} icon={FileText} variant="warning" />
