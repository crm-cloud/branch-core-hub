...
export async function createPOSSale(sale: {
  ...
  guestEmail?: string;
  /**
   * If true: invoice is created as `pending` with NO payment row.
   * Webhook (payment-webhook) will call record_payment to mark paid.
   */
  awaitingPayment?: boolean;
  /** Optional flat discount amount (post-validation, e.g. coupon value). */
  discountAmount?: number;
  /** The coupon code (if any) — recorded on the invoice notes & usage counter incremented. */
  discountCodeId?: string;
  discountCode?: string;
  /**
   * Wallet portion (₹) to apply against the bill. The remainder is charged via `paymentMethod`.
   * Wallet must already be deducted by the caller? No — we deduct here for atomicity.
   */
