-- Fix create_pos_sale: invoices.source is plain text, not an enum.
-- The previous version cast 'pos'::invoice_source which fails with
-- "type invoice_source does not exist". Also keep the rest of the logic
-- (wallet, coupon, inventory, settlement, idempotency) intact.
...
BEGIN
  ...
  -- Idempotency replay: same key → return prior result instead of duplicating
  ...
  -- IMPORTANT: invoices.source is plain text — do NOT cast to invoice_source enum.
...
-- Mark the older auto-logged errors that are addressed by this sprint as resolved
-- so System Health doesn't keep showing them as fresh issues.
UPDATE public.error_logs
   SET status = 'resolved', resolved_at = now()
 WHERE status = 'open'
   AND (
        error_message ILIKE '%invoice_source%does not exist%'
     OR error_message = 'Payment gateway not configured'
     OR error_message = 'Payment error: Edge Function returned a non-2xx status code'
     OR error_message ILIKE 'Cannot coerce the result to a single JSON object%profiles%'
   );