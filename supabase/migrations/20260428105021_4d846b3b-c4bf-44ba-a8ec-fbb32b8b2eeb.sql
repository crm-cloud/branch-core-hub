-- 1) Fix create_pos_sale: guard the coupon update with a boolean flag instead
-- of dereferencing v_code_row.id (which raises 'record not assigned yet'
-- when no coupon was applied).
...
BEGIN
  ...
  IF p_idempotency_key IS NOT NULL THEN
    ...
    WHERE i.notes IS NOT NULL
      AND i.notes LIKE '%' || p_idempotency_key || '%'
    ORDER BY i.created_at DESC
    LIMIT 1;
    IF v_existing_invoice_id IS NOT NULL THEN
      RETURN jsonb_build_object(
        'pos_sale_id', v_existing_pos_sale_id,
  ...
  -- Coupon usage update — guarded by the boolean flag, NOT by v_code_row.id
...
-- 2) Mark resolved error logs for this v_code_row issue
UPDATE public.error_logs
   SET status = 'resolved', resolved_at = now()
 WHERE status = 'open'
   AND error_message ILIKE '%v_code_row%not assigned%';

-- 3) Atomic PT package purchase for members (pending state until paid online).
...
BEGIN
  ...
  SELECT id, name, price, sessions_count, validity_days
    INTO v_pkg
  FROM pt_packages
  ...
  SELECT p.full_name, p.phone, p.email INTO v_member_profile
  FROM members m LEFT JOIN profiles p ON p.id = m.user_id
  WHERE m.id = p_member_id;

  -- Pending invoice — payment webhook flips status & activates package.
  INSERT INTO invoices (
    ...
    v_today, 'pt_package',
    'PT Package Purchase: ' || v_pkg.name ||
  ...
  INSERT INTO invoice_items (
    ...
    v_invoice_id, v_pkg.name || ' (' || COALESCE(v_pkg.sessions_count, 0) || ' sessions)',
    1, v_total, v_total, 'pt_package', v_pkg.id
  );

  -- Member PT package row in pending state (sessions_remaining = 0 until paid).
  INSERT INTO member_pt_packages (
    member_id, package_id, branch_id, trainer_id,
    sessions_total, sessions_used, sessions_remaining,
    purchase_date, expiry_date, status,
    invoice_id
  ) VALUES (
    p_member_id, v_pkg.id, p_branch_id, p_trainer_id,
    COALESCE(v_pkg.sessions_count, 0), 0, 0,
    v_today, v_today + COALESCE(v_pkg.validity_days, 90),
    'pending', v_invoice_id
  ) RETURNING id INTO v_member_pkg_id;

  RETURN jsonb_build_object(
    'success', true,
    'invoice_id', v_invoice_id,
    'member_pt_package_id', v_member_pkg_id,
    'total', v_total
  );
