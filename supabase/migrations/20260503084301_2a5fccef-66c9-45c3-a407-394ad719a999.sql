-- 1) Add customer fields to invoices
...
CREATE INDEX IF NOT EXISTS idx_invoices_customer_phone
  ON public.invoices (customer_phone)
  WHERE customer_phone IS NOT NULL;

-- 2) Backfill from pos_sales
UPDATE public.invoices i
   SET customer_name  = COALESCE(i.customer_name,  ps.customer_name),
       customer_email = COALESCE(i.customer_email, ps.customer_email),
       customer_phone = COALESCE(i.customer_phone, ps.customer_phone)
  FROM public.pos_sales ps
 WHERE ps.invoice_id = i.id
   AND (i.customer_name IS NULL OR i.customer_email IS NULL OR i.customer_phone IS NULL);

-- 3) Update create_pos_sale to populate the new invoice columns
